Конечно! Вот неформальный райтап:

---

# Writeup: wirepass (Network Forensics) — KubSTU
...
Дамп сети. По легенде — перехваченная передача оперативных документов Пингвиньего Командования. Задача — найти флаг.
...
1. Ищи нестандартные порты — там кастомные протоколы
2. В FTP смотри не только команды USER/PASS, но и **все строки** во всём pcap
3. Магик в начале пакета (`XFER`, `PING`, `HEAD` и т.д.) → кастомный протокол → реверси структуру
4. ZIP с AES-256 → метод 99, WinZip AES → PBKDF2 + CTR с LE-счётчиком
5. Если пароль не очевидный — ищи его по всему трафику строками